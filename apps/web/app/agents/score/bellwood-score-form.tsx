    ...
    <div>
      <form onSubmit={onSubmit} className="space-y-4">
        <div className="grid gap-4 md:grid-cols-2">
          <label className="block">
            ...
            <input
              ...
              required
              placeholder="M14 5LL"
              className="mt-2 w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-[15px] font-mono uppercase outline-none focus:border-[#C6A664]"
            />
          </label>
          <label className="block">
            <span className="font-mono text-[11px] uppercase tracking-[0.18em] text-slate-600">
            ...
            <input
              ...
              className="mt-2 w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-[15px] outline-none focus:border-[#C6A664]"
            />
          </label>
        </div>
        ...
        <button
          ...
          className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#0A2540] px-7 py-3.5 text-sm font-medium text-white transition hover:bg-[#13365c] disabled:cursor-not-allowed disabled:opacity-50 md:w-auto"
        >
      ...
      {result && result.ok && (
        <div className="mt-8 rounded-2xl border-2 border-[#C6A664]/40 bg-[#FAF6EA] p-6 md:p-8">
          <p className="font-mono text-[10px] uppercase tracking-[0.22em] text-[#C6A664]">
            Indicative range
          </p>
          <p className="mt-2 font-serif text-3xl font-semibold tracking-[-0.02em] text-[#0A1020] md:text-5xl">
          ...
          <dl className="mt-6 grid grid-cols-2 gap-3 text-[12px] md:grid-cols-4">
            <div>
              <dt className="text-slate-500">AVM low</dt>
              <dd className="font-mono text-[13px] font-semibold text-[#0A1020]">
                {formatGBP(result.avmMinPence)}
              </dd>
            </div>
            <div>
              <dt className="text-slate-500">AVM high</dt>
              <dd className="font-mono text-[13px] font-semibold text-[#0A1020]">
            ...
            <div>
              ...
              <dd className="font-mono text-[13px] font-semibold capitalize text-[#0A1020]">
                {result.marketBand ?? '—'}
              </dd>
            </div>
          </dl>
          ...
          {result.reasoning.length > 0 && (
            <details className="mt-5">
              <summary className="cursor-pointer font-mono text-[11px] uppercase tracking-wider text-slate-500 hover:text-[#0A1020]">
          ...
          <div className="mt-6 border-t border-amber-200 pt-4 text-[12px] leading-relaxed text-slate-600">
            <p>
          ...
          <div className="mt-5 flex flex-col gap-3 sm:flex-row">
            <a
              href="/save-the-sale"
              className="inline-flex items-center justify-center gap-2 rounded-full bg-[#C6A664] px-6 py-3 text-sm font-medium text-[#0A1020] transition hover:bg-[#b08f52]"
            >
              Send this property to us →
            </a>
            <a
              href="/instant-offer/methodology"
              className="inline-flex items-center justify-center gap-2 rounded-full border border-slate-300 bg-white px-6 py-3 text-sm text-slate-700 transition hover:border-slate-400"
            >
              See methodology
            </a>
          </div>
