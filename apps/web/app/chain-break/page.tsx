...
export const metadata: Metadata = {
  title: 'Chain just broken? · Bellwoods Lane',
  description:
    'A real cash figure on screen in 60 seconds. Signed binding offer to your client in 4 working hours. For UK estate agents who just had an exchange collapse.',
  openGraph: {
    title: 'Chain just broken? · Bellwoods Lane',
    description:
      'Cash figure in 60 seconds, signed offer in 4 hours. For UK estate agents.',
    type: 'website',
  },
};
...
export default function ChainBreakPage() {
  return (
    ...
    >
      ...
      {/* ————— HERO + FORM (single viewport on desktop) ————— */}
        ...
        <div className="mx-auto grid max-w-6xl grid-cols-1 gap-12 lg:grid-cols-[1fr_1.2fr] lg:gap-16">
          <div>
            ...
            >
              Chain just broken?
              <br />
              <span className="italic text-[#C6A664]">Save the deal.</span>
            </h1>
            <p className="mt-7 max-w-md text-lg leading-relaxed text-slate-600">
              A real cash figure on screen in 60 seconds, drawn from HM Land
              Registry comps. A signed, binding offer document in your
              client&rsquo;s inbox within 4 working hours.
            </p>
            <ul className="mt-8 space-y-3 text-[14px] text-slate-700">
              ...
              <li className="flex items-start gap-3">
                <span className="mt-1 inline-block h-1.5 w-1.5 shrink-0 rounded-full bg-[#C6A664]" />
                <span>
                  <strong className="text-[#0A1020]">14-day completion if needed.</strong>{' '}
                  Or paced to your client&rsquo;s onward purchase date.
                </span>
              </li>
              <li className="flex items-start gap-3">
                <span className="mt-1 inline-block h-1.5 w-1.5 shrink-0 rounded-full bg-[#C6A664]" />
            ...
            <p className="mt-8 text-[12px] text-slate-500">
              Prefer email?{' '}
              <a
                href="mailto:hello@bellwoodslane.co.uk?subject=Chain%20break%20%E2%80%94%20urgent"
          ...
          <div>
            <AgentQuickForm defaultSituation="chain_break" />
          </div>
        </div>
      </section>
