...
export function VersionHistory({
  ...
  const handleRestore = (version: number) => {
    startTransition(async () => {
      try {
        const { version: newVersion } = await restoreVersion(version);
  ...
  if (versions.length === 0) {
    return (
      <section className="rounded-xl border border-dashed bg-slate-50 p-5 text-muted-foreground text-xs">
        No saved versions yet. The scorer is running on built-in defaults. Save
        a config above to start a version history.
      </section>
    );
  }
